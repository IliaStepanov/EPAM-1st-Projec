...
<div class="topnav">
    ...
    <c:if test="${sessionUser.isAdmin()}"> <a href="<%=EndPoints.USER + EndPoints.ALL + EndPoints.FIRST_PAGE%>">
        <spring:message code="lang.userDAO"/></a></c:if>
    <c:if test="${sessionUser.isAdmin()}"> <a href="<%=EndPoints.PLANE + EndPoints.ALL%>"> <spring:message
            code="lang.planeDAO"/></a></c:if>
    <c:if test="${sessionUser.isAdmin()}"> <a href="<%=EndPoints.FLIGHTS + EndPoints.ALL + EndPoints.FIRST_PAGE%>">
        <spring:message code="lang.flightDAO"/></a></c:if>
    <c:if test="${sessionUser.isAdmin()}"> <a href="<%=EndPoints.TICKETS + EndPoints.ALL%>"> <spring:message
            code="lang.ticketDAO"/></a></c:if>
    <c:if test="${sessionUser.isAdmin()}"> <a href="<%=EndPoints.AIRPORT + EndPoints.ALL%>"> <spring:message
            code="lang.airportDAO"/></a></c:if>
